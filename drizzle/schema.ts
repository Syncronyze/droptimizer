...
export const encounters = pgTable('encounters', {
    ...
    id: bigint('id', { mode: 'number' }).primaryKey().notNull(),
    // You can use { mode: "bigint" } if numbers are exceeding js number limitations
    instance_id: bigint('instance_id', { mode: 'number' })
        .notNull()
...
export const encounter_items = pgTable(
    ...
    (table) => {
        return {
            ...
            encounter_items_encounter_id_key: unique('encounter_items_encounter_id_key').on(
                table.encounter_id,
            ),
...
export const class_instances = pgTable(
    'class_instances',
    {
        // You can use { mode: "bigint" } if numbers are exceeding js number limitations
        class_id: bigint('class_id', { mode: 'number' })
    ...
    (table) => {
        return {
            class_instances_pkey: primaryKey({
                columns: [table.class_id, table.instance_id],
                name: 'class_instances_pkey',
            }),
            class_instances_class_id_key: unique('class_instances_class_id_key').on(table.class_id),
            class_instances_instance_id_key: unique('class_instances_instance_id_key').on(
                table.instance_id,
            ),
        };
    },
...
export const spec_reports = pgTable(
    'spec_reports',
    {
        // You can use { mode: "bigint" } if numbers are exceeding js number limitations
        char_id: bigint('char_id', { mode: 'number' })
            .notNull()
            .references(() => characters.id),
        // You can use { mode: "bigint" } if numbers are exceeding js number limitations
        spec_id: bigint('spec_id', { mode: 'number' })
            .notNull()
            .references(() => specs.id),
        difficulty: text('difficulty').notNull(),
    ...
    (table) => {
        return {
            spec_reports_pkey: primaryKey({
                columns: [table.char_id, table.spec_id, table.difficulty],
                name: 'spec_reports_pkey',
            }),
        };
    },
);
